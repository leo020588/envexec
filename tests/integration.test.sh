...
EOF
  ...
  if bw_prepare_session; then
    ...
    expect_success "bw source dangerous-print run ($BW_ITEM / $BW_FOLDER_NAME)" \
      bash "$ENVEXEC" --from-bw "$BW_ITEM" --bw-folder "$BW_FOLDER_NAME" --dangerously-print-env
    expect_success "bw source write outputs ($BW_ITEM / $BW_FOLDER_NAME)" \
      bash "$ENVEXEC" --from-bw "$BW_ITEM" --bw-folder "$BW_FOLDER_NAME" --write-env "$bw_parsed" --write-raw "$bw_raw" -- true

    if bash "$ENVEXEC" --from-bw "$BW_ITEM" --bw-folder "$BW_FOLDER_NAME" -- true >"$quiet_stdout" 2>"$quiet_stderr"; then
      if [[ ! -s "$quiet_stdout" ]] && [[ ! -s "$quiet_stderr" ]]; then
    ...
    if bash "$ENVEXEC" --from-bw "$BW_ITEM" --bw-folder "$BW_FOLDER_NAME" --debug -- true >"$quiet_stdout" 2>"$quiet_stderr"; then
...
' "$bw_expected_parsed" >"$bw_expected_json_file"
    ...
    expect_success "bw helper validates expected injected variables" \
      bash "$ENVEXEC" --from-bw "$BW_ITEM" --bw-folder "$BW_FOLDER_NAME" -- env "EXPECTED_ENV_JSON=$bw_expected_json" node "$TEST_JS"
  else
